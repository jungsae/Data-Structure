...
SparsePolynomial SparsePolynomial::Add(const SparsePolynomial &poly)
{
	...
	while ((l < this->num_terms_) && (r < poly.num_terms_))
	{
		if (this->terms_[l].exp == poly.terms_[r].exp)
		{
			temp.NewTerm(this->terms_[l].coef + poly.terms_[r].coef, this->terms_[l].exp);
			l++, r++;
		}
		else if (this->terms_[l].exp > poly.terms_[r].exp)
		{
			temp.NewTerm(poly.terms_[r].coef, poly.terms_[r].exp);
			r++;
		}
		else
		{
			temp.NewTerm(this->terms_[l].coef, this->terms_[l].exp);
			l++;
		}
	}
